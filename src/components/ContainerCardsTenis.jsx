      ...
      {produtos.map((produto) => (
        <div key={produto.id} className=" ">
          ...
          <div className="bg-white p-4 rounded shadow hover:shadow-lg transition">
            ...
            <div className="mt-4 text-sm text-gray-500">{produto.categoria}</div>
            <div className="text-base font-medium text-gray-800">
              <h3 className="font-medium mb-1">{produto.nome}</h3>
            </div>
            <div className="text-sm mt-1">
              <span className="line-through text-gray-400 mr-2">{produto.preco}</span>
              <span className="font-bold text-gray-900">{produto.precoDesconto}</span>
            </div>
          </div>
        </div>
      ))}
